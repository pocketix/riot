...
const DevicesSection: React.FC<DevicesSectionProps> = (props) => {
  return <div className={styles.section}>{props.devicesQueryData && props.devicesQueryData.devices.map((device) => <DeviceWidget key={device.id} name={device.name} uid={device.uid} typeDenotation={device.type.denotation}></DeviceWidget>)}</div>
}

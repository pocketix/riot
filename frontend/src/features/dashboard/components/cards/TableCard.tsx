...
export const TableCard = ({ cardID, title, layout, setLayout, cols, breakPoint, editModeEnabled, handleDeleteItem, width, height, setHighlightedCardID, data }: TableCardProps) => {
                    ...
                    <thead className='border-b-[2px]'>
                        <tr>
                            ...
                            {data.columns.map((column, index) => (
                                <th key={index} className='text-center text-xs'>{column.header}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {data.rows.map((row, rowIndex) => (
                            <tr key={rowIndex}>
                                ...
                                {row.values.map((value, valueIndex) => (
                                    <td key={valueIndex} className='text-sm text-center'>{value.value}</td>
                                ))}
                            </tr>
                        ))}
                    </tbody>

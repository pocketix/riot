    ...
    <Popover open={open} onOpenChange={setOpen} modal={true}>
      <PopoverTrigger asChild>
        <Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between flex">
          {selectedParameter ? parameters?.find((parameter) => parameter.denotation === selectedParameter.denotation)?.denotation : 'Select a parameter'}
              ...
              <CommandGroup>
                {parameters?.map((parameter) => (
                  ...
                  >
                    ...
                    <Check className={cn('ml-auto', selectedParameter?.denotation === parameter.denotation ? 'opacity-100' : 'opacity-0')} />
                  </CommandItem>
                ))}
              </CommandGroup>
            </div>

...
const EditableTree: React.FC<EditableTreeProps> = (props) => {
  ...
  const updateTreePositionWithinContainer = useCallback(
    (containerWidth: number, containerHeight: number) => {
      const treeWidth: number = calculateTreeWidthInPixels(props.editableTreeNodeData) // FIXME: Does not work in all cases...
      const treeHeight: number = calculateTreeHeightInPixels(props.editableTreeNodeData)
      const x: number = (containerWidth - treeWidth) / 2 + treeWidth / 2 // FIXME: (treeWidth / 2) is not precise enough...

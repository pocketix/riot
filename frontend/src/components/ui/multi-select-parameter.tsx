 ...
 */
const ParameterMultiSelectVariants = cva('m-1 transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300', {
  variants: {
    variant: {
              ...
              <div className="flex justify-between items-center w-full">
                <div className="flex flex-wrap items-center">
                  {selectedValues.slice(0, maxCount).map((value) => {
                    ...
                    return (
                      <Badge key={value} className={cn(isAnimating ? 'animate-bounce' : '', ParameterMultiSelectVariants({ variant }))} style={{ animationDuration: `${animation}s` }}>
                  ...
                  {selectedValues.length > maxCount && (
                    <Badge
                      className={cn('bg-transparent text-foreground border-foreground/1 hover:bg-transparent', isAnimating ? 'animate-bounce' : '', ParameterMultiSelectVariants({ variant }))}
                ...
                {options.map((option) => {
                  ...
                  return (
                    <CommandItem key={option.value} onSelect={() => toggleOption(option.value)} className="cursor-pointer">
                      <div className={cn('mr-2 flex h-4 w-4 items-center justify-center rounded-sm', isSelected ? 'text-primary' : 'opacity-50 [&_svg]:invisible')}>
                        <CheckIcon className="h-4 w-4" />
                      </div>
